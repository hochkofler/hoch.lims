...
import hoch.lims.permissions as permissions
from hoch.lims import PRODUCT_NAME
...
BEHAVIORS = [
    ("SampleMatrix", [
         "hoch.lims.extender.samplematrix.ISampleMatrixSchemaExtender",
         "hoch.lims.extender.behaviors.variablestable.IVariablesTableBehavior",
     ]),
]
...
def setup_handler(context):
    ...
    # Add additional behaviors to existing content types
...
def setup_behaviors(portal):
    ...
    for portal_type, behavior_ids in BEHAVIORS:
        ...
        if additional:
            ...
            logger.info("Added behaviors '%s' to type '%s'"
                        % (', '.join(additional), portal_type))

def add_folder(portal):
    """Adds the initial folders
    """
